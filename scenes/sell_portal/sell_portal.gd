...
func _sell_unit(unit: Unit) -> void:
	print("Selling Tier %s %s:" % [unit.stats.tier, unit.stats.name])
	print("\tGold before sale: %s" % player_stats.gold)
	var unit_value := unit.stats.get_gold_value()
	print("\tTier %s %s's value: %s" % [unit.stats.tier, unit.stats.name, unit_value])
	player_stats.gold += unit_value
	# TODO: give items back to item pool
	# TODO: put unit back to the pool
	print("\tGold after sale: %s" % player_stats.gold)
	unit.queue_free()
